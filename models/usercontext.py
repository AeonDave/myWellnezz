...
from modules.math_util import obfuscate, get_key, write_obfuscation, read_obfuscation
...
class UserContext:
    def __init__(self, **kwargs):
        ...
        self.token_expire: datetime = None if not kwargs.get('token_expire') else datetime.fromisoformat(
            kwargs.get('token_expire'))
        self.token_gen: datetime = datetime.now() if not kwargs.get('token_gen') else datetime.fromisoformat(
            kwargs.get('token_gen'))
    ...
    async def login_app(self):
        url = schema + 'services.' + base_url + f'/Application/{app_id}/Login'
        headers = {
