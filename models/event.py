...
async def action_event(user: UserContext, event: Event) -> bool:
    url = schema + 'services.' + base_url + api_book_app \
          + f'/{event.id}/{"book" if not event.is_participant else "unbook"}'
    headers = {
...
async def update_events(user: UserContext, facility: Facility, start: int) -> dict[str, Event]:
    url = schema + 'services.' + base_url + api_search_app + f'/{facility.id}/SearchCalendarEvents'
    while True:
        ...
        try:
            ...
            response = await async_post(url, headers, payload)
            if not response and not response['data']:
                print(f'Something bad happened: {response.status}')
                user.token = None
